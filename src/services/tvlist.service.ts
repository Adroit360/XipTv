...
export class TvListService implements OnInit {
    ...
    constructor(private httpClient: HttpClient,
        ...
        this.tvLinksCredentialsSubject.subscribe(response => {
            let oldCredentials = appStorage.getString("Credentials");
            let newCredentialsString = JSON.stringify(response);
            console.log(newCredentialsString);
            if(oldCredentials == newCredentialsString){
                console.log("Not Replacing");
                return;
            }else{
                console.log("Replacing");
                appStorage.setString("Credentials",newCredentialsString);
            }
    ...
    getCredentials() {
        ...
        this.httpClient.get<{ username, password, sampleurl, }>(`${settings.baseUri}/tvlist/checklink`)
            .subscribe(response => {
                ...
                this.routerExtensions.navigate(['no-internet'],{
                    clearHistory:true
                });
            });
    }
    ...
    isRemoteLinksChanged() {
        ...
        this.httpClient.get<any>(`${settings.baseUri}/misc/getCounter`)
            .subscribe(response => {
                appStorage.setString("Counter", response.toString());
    ...
    resetLocalData() {
        ...
        this.miscService.alert("StreamLinks Updated","Login again to get a revamped streaming experience");  
    }
